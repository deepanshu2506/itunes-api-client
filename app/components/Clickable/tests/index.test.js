...
describe('<Clickable /> component tests', () => {
  ...
  it('should contain render the text according to the textId', () => {
    const { getAllByText } = renderWithIntl(<Clickable textId="iTunes_search" />);
  ...
  it('should call the prop onClick when the clickable component is clicked', () => {
    ...
    const { getAllByText, queryByText } = renderWithIntl(<Clickable onClick={clickSpy} textId="iTunes_search" />);
    expect(getAllByText(/iTunes Search/).length).toBe(1);
    fireEvent.click(queryByText(/iTunes Search/));
